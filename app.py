...
/* Login */
.login-wrap {{
  position:fixed; inset:0; display:flex; align-items:center; justify-content:center; background: var(--body);
}}
.login-card {{
  width:min(480px, 92vw); background:#FFFFFF; border:1px solid #E3EDF6; border-radius:16px; padding:28px 24px; box-shadow:0 10px 24px rgba(2,26,52,0.06);
}}
.login-logo {{ display:flex; align-items:center; justify-content:center; margin-bottom:14px; }}
.login-logo img {{ width:120px; height:auto; }}
.login-title {{ text-align:center; font-weight:800; color:{TITLE_DARK}; font-size:22px; margin-bottom:8px; }}
.login-sub {{ text-align:center; color:#6F8198; font-size:12px; margin-bottom:10px; }}
"""
...
# =========================================================
def login_screen():
  # UI centrada, dos inputs y logo Wayki (sin tocar tamaños/colores del resto)
  st.markdown('<div class="login-wrap">', unsafe_allow_html=True)
  st.markdown('<div class="login-card">', unsafe_allow_html=True)
  st.markdown('<div class="login-logo"><img src="logo-wayki.png" alt="Wayki Consulting"></div>', unsafe_allow_html=True)
  st.markdown(f'<div class="login-title">Bienvenido a <span style="color:{PRIMARY}">SelektIA</span></div>', unsafe_allow_html=True)
            ...
            if not uploaded_files:
                ...
                try:
                    ...
                    for uf in uploaded_files:
                        ...
                        meta = _extract_with_azure(job_desc, cv_text)
        ...
        with cols[i]:
            ...
            for c in candidates_by_stage[stage]:
                ...
                with st.form(key=f"form_move_{c['id']}", clear_on_submit=False):
                    ...
                    # -------- FIX: índice seguro para evitar ValueError --------
                    try:
                        target = PIPELINE_STAGES[min(PIPELINE_STAGES.index(stage)+1, len(PIPELINE_STAGES)-1)]
                        default_idx = available_stages.index(target) if target in available_stages else 0
                    except Exception:
                        default_idx = 0
                    new_stage = st.selectbox("Mover a:", available_stages, index=default_idx, key=f"select_move_{c['id']}", label_visibility="collapsed")
