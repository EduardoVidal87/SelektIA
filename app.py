...
def page_analytics():
  ...
  # Paleta: verde principal y variantes (apilados)
  MINT      = PRIMARY                 # "#00CD78" en tu base
  MINT_400  = "#6EE7B7"
  MINT_200  = "#B7F3DC"
  AREA_FILL = "rgba(0,205,120,0.18)"
  ...
  def kpi_card(title, value, series, fmt="{:,.0f}", help_text=None):
    ...
    with c:
      ...
      with col1:
        st.markdown(f"**{title}**")
        st.markdown(
          f"<div style='font-size:34px;font-weight:800;color:{TITLE_DARK}'>{fmt.format(value)}</div>",
          unsafe_allow_html=True,
        )
        if help_text:
      ...
      with col2:
        if series is not None and len(series) > 1:
          spark = px.area(x=list(range(len(series))), y=series, height=70)
          spark.update_traces(hovertemplate=None, line_color=MINT, fillcolor=AREA_FILL)
          spark.update_layout(
  ...
  # -------------------------------
  # Construimos un DF con toda la info necesaria desde ss.candidates
  cand_rows = []
  for c in ss.candidates:
    # FIT (si no viene en el objeto, lo calculamos)
    fit = c.get("Fit")
    if fit is None:
    ...
    # Canal (prefijo de nombre importado; si no, "Otros")
    name = c.get("Name", "")
    canal = name.split("_")[0] if "_" in name else "Otros"

    # Fecha de actualización (para sparklines)
    meta = c.get("meta", {})
    ...
    cand_rows.append({"Name": name, "fit": int(fit), "canal": canal, "fecha": fch})

  dfc = pd.DataFrame(cand_rows) if cand_rows else pd.DataFrame(columns=["Name", "fit", "canal", "fecha"])

  # Sparklines (conteo y fit medio por día), con relleno de días faltantes
  if not dfc.empty:
    idx = pd.date_range(dfc["fecha"].min(), date.today(), freq="D")
    series_count = dfc.groupby("fecha").size().reindex(idx, fill_value=0).astype(int)
    series_fit   = dfc.groupby("fecha")["fit"].mean().reindex(idx, fill_value=0)
  ...
  with k1:
    kpi_card("Findem-Influenced Applications", total_cvs, spark_counts)
  with k2:
    rate = (dfc["fit"] >= 70).mean() * 100 if not dfc.empty else 0
    kpi_card("Contact-to-interest Rate", rate, spark_fit, fmt="{:.1f}%")
  with k3:
    kpi_card("Puestos activos", total_puestos, spark_counts, fmt="{:,.0f}")
  ...
  # (1) Barras apiladas por Hiring Channel (bandas de fit)
  with g1:
    st.markdown("**Findem-influenced Application by Hiring Channel**")
    if not dfc.empty:
      dfc["band"] = pd.cut(
        dfc["fit"], bins=[-1, 39, 69, 100], labels=["Bajo (<40)", "Medio (40-69)", "Alto (≥70)"]
      )
      ...
      # Orden de bandas para apilado y colores menta
      band_cat = pd.Categorical(
        dist["band"], categories=["Bajo (<40)", "Medio (40-69)", "Alto (≥70)"], ordered=True
      )
      dist = dist.assign(band=band_cat).sort_values(["canal", "band"])

      fig = px.bar(
        dist, x="canal", y="pct", color="band",
        color_discrete_sequence=[MINT_200, MINT_400, MINT],
        labels={"pct": "%"}, height=340
        )
  ...
  with g2:
    ...
    if not dfc.empty:
      ...
      funnel_df  = pd.DataFrame(
        {"Stage": ["Contacted", "Replied", "Interested"], "Count": [contacted, replied, interested]}
      )
      fig = px.funnel(funnel_df, x="Count", y="Stage", height=340, color_discrete_sequence=[MINT])
      fig.update_layout(
  ...
  # (3) Días abiertos por puesto (proxy visual de “time to reply”)
  with g3:
  ...
  with s1:
    ...
    if not dfc.empty:
      reply = dfc.assign(reply=dfc["fit"] >= 40).groupby("canal")["reply"].mean().reset_index()
      fig = px.bar(reply, x="canal", y="reply", height=320,
                   labels={"reply": "Reply Rate"},
                   color_discrete_sequence=[MINT])
  ...
  with s2:
    ...
    if not dfc.empty:
      ir = dfc.assign(interested=dfc["fit"] >= 70).groupby("canal")["interested"].mean().reset_index()
...
ROUTES = {
  ...
  "interview": page_eval, "offer": page_eval, "onboarding": page_eval, "hh_tasks": page_agent_tasks,
  "agents": page_agents, "flows": page_flows, "agent_tasks": page_agent_tasks,
  "analytics": page_analytics, "create_task": page_create_task,
}
