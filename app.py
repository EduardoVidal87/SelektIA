...
def extract_text_from_file(uploaded_file) -> str:
    ...
    Importante: usa el file-like actual. NO consume el buffer que guardamos para visor.
    """
    try:
...
with st.sidebar:
    ...
    if files:
        ...
        for f in files:
            # guardamos bytes para visor ANTES de leer en extract
            f.seek(0)
            raw_bytes = f.read()
            # volvemos al inicio para extraer texto
            f.seek(0)
        ...
        # cuando cambian candidatos, resetea pipeline
...
# --------------------------------------------------------------------------------------
# TAB 2: EVALUACIÓN DE CVS (base + pipeline)
# --------------------------------------------------------------------------------------
with tab_eval:
    ...
    if not st.session_state.candidates:
        ...
        # =========================
        # NUEVO: Pipeline Sourcing
        # =========================
        st.markdown("### Pipeline de candidatos")
        st.caption("Avanza etapas, registra contacto, fuente y notas. (Se guarda en la sesión actual).")

        etapas = ["Leads", "Contactado", "Aplicante", "Entrevista (Gerencia)", "Oferta", "Archivado"]

        # construir pipeline si no existe (o si cambió el set)
