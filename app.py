...
.agent-card {{
  ...
  padding:16px;
  text-align:center;
  box-shadow:0 6px 18px rgba(14,25,43,.06);
}}
.agent-avatar {{
  width:120px;height:120px;border-radius:999px;object-fit:cover;border:4px solid #F1F7FD;margin:0 auto 10px auto;
}}
.agent-role-title {{ font-weight:800;color:{TITLE_DARK};font-size:16px }}
.agent-sub {{ font-size:12px;opacity:.8;margin-top:6px }}
...
# Ajustes puntuales que ya tenías (brand-sub tamaño / espacio logo / sidebar compacto)
st.markdown("""
<style>
...
[data-testid="stSidebar"] .sidebar-brand{ margin-bottom: 55px !important; }
...
# Roles persistentes (defaults + custom guardados en data/roles.json)
...
if "new_role_mode" not in ss: ss.new_role_mode = False   # toggle "Nuevo" en página Agentes
...
def page_agents():
  ...
  if ss.new_role_mode:
    with st.form("new_agent_form", clear_on_submit=False):
      # Rol editable (elegir existente o escribir uno nuevo)
      roles = ss.roles
      rol_sel = st.selectbox("Rol existente", roles, index=0)
      new_rol = st.text_input("… o escribe un nuevo rol (se guardará)", value="")
      ...
      if ok:
        final_role = new_rol.strip() if new_rol.strip() else rol_sel
        if final_role not in ss.roles:
          ss.roles.append(final_role)
          save_roles(ss.roles)
        ss.agents.append({
          "rol": final_role, "objetivo": objetivo, "backstory": backstory,
  ...
  # ----- Tarjetas de agentes (ABAJO) -----
  if not ss.agents:
    st.info("Aún no hay agentes. Crea el primero con **➕ Nuevo**.")
    return

  # Render en grilla de 5 por fila (mantiene el orden FIFO)
  CARDS_PER_ROW = 5
  total = len(ss.agents)
    ...
    for j, ag in enumerate(row_agents):
      with cols[j]:
        ...
        <div class="agent-card">
          ...
          <div class="agent-role-title">{ag.get('rol','—')}</div>
          <div class="agent-sub">{ag.get('objetivo','—')}</div>
        </div>
        """, unsafe_allow_html=True)

        # Toolbar centrada (un solo bloque)
        st.markdown('<div class="toolbar">', unsafe_allow_html=True)
        c1, c2, c3, c4 = st.columns(4)
        idx = start + j
        with c1:
          if st.button("👁", key=f"ag_view_{idx}", help="Ver"):
            ss.agent_view_open[idx] = not ss.agent_view_open.get(idx, False)
            st.rerun()
        with c2:
          if st.button("✏", key=f"ag_edit_{idx}", help="Editar"):
            ss.agent_edit_open[idx] = not ss.agent_edit_open.get(idx, False)
            st.rerun()
        with c3:
        ...
        if ss.agent_view_open.get(idx, False):
          st.markdown("**Detalle**")
          st.markdown(f"- **Objetivo:** {ag.get('objetivo','—')}")
          st.markdown(f"- **Backstory:** {ag.get('backstory','—')}")
          st.markdown(f"- **Guardrails:** {ag.get('guardrails','—')}")
          st.markdown(f"- **Herramientas:** {', '.join(ag.get('herramientas',[])) or '—'}")
          st.markdown(f"- **Modelo LLM:** {ag.get('llm_model','—')}")
          st.markdown(f"- **Permisos:** {', '.join(ag.get('perms',[])) or '—'}")

        # Panel editar
        if ss.agent_edit_open.get(idx, False):
          st.info(f"Editando: {ag.get('rol')}")
          with st.form(f"agent_edit_form_{idx}"):
            rol = st.text_input("Rol*", value=ag.get("rol",""))
            objetivo  = st.text_input("Objetivo*", value=ag.get("objetivo",""))
            backstory = st.text_area("Backstory*", value=ag.get("backstory",""), height=100)
            guardrails= st.text_area("Guardrails", value=ag.get("guardrails",""), height=80)
