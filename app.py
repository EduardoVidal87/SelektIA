...
st.markdown("#### TAREAS")
for txt, sec in [("Todas las tareas","tasks_all"), ("Asignadas a mí","tasks_me"), ("Asignadas a mi equipo","tasks_team"), ("Archivadas","tasks_archived")]:
 ...
 for txt, sec in [("Publicación & Sourcing","def_carga"), ("Puestos","puestos"), ("Evaluación de CVs","eval"),
...
def page_pipeline():
 ...
 if not ss.candidates:
 st.info("Primero carga CVs en **Publicación & Sourcing**.")
 return
...
# =========================================================
# NUEVO DASHBOARD ANALYTICS (look & feel moderno)
# =========================================================
def _ana_css():
 st.markdown("""
 <style>
 .ana-card{background:#FFFFFF;border-radius:12px;padding:16px;border:1px solid rgba(20,36,51,.06);box-shadow:0 1px 3px rgba(0,0,0,.06),0 1px 2px rgba(0,0,0,.04)}
 .ana-kpi-title{font-weight:600;color:%s;margin:0 0 6px 0}
 .ana-kpi-value{font-size:28px;font-weight:800;color:%s}
 .ana-kpi-sub{font-size:12px;color:#57708A}
 </style>
 """ % (TITLE_DARK, TITLE_DARK), unsafe_allow_html=True)

def _ana_spark(series):
 if not series: series=[0,0,0,0,0,0,0]
 fig=go.Figure(); fig.add_trace(go.Scatter(y=series,mode="lines",line=dict(width=2),hoverinfo="skip"))
 fig.update_layout(height=60,margin=dict(l=0,r=0,t=0,b=0),xaxis=dict(visible=False),yaxis=dict(visible=False))
 return fig

def _ana_kpi(col, title, value, sub="", series=None):
 with col:
  st.markdown('<div class="ana-card">', unsafe_allow_html=True)
  st.markdown(f'<div class="ana-kpi-title">{title}</div>', unsafe_allow_html=True)
  st.markdown(f'<div class="ana-kpi-value">{value}</div>', unsafe_allow_html=True)
  if sub: st.markdown(f'<div class="ana-kpi-sub">{sub}</div>', unsafe_allow_html=True)
  st.plotly_chart(_ana_spark(series), use_container_width=True, config={"displayModeBar": False})
  st.markdown("</div>", unsafe_allow_html=True)

def _df_from_state(key, cols):
 df=ss.get(key)
 if isinstance(df, pd.DataFrame) and set(cols).issubset(df.columns): return df.copy()
 return pd.DataFrame(columns=cols)

def _section_sourcing_perf():
 _ana_css()
 st.subheader("Sourcing Performance")
 apps_by_channel=_df_from_state("analytics_apps_by_channel",["date","channel","applications"])
 outreach_funnel=_df_from_state("analytics_outreach_funnel",["stage","count"])
 reply_rate=_df_from_state("analytics_reply_rate",["channel","rate"])
 cti_rate=_df_from_state("analytics_cti_rate",["channel","rate"])

 total_apps=int(ss.get("kpi_total_apps", apps_by_channel["applications"].sum() if not apps_by_channel.empty else 0))
 cti=float(ss.get("kpi_cti",0.0))
 ttr=float(ss.get("kpi_ttr_days",0.0))

 s_apps=ss.get("spark_apps", list(apps_by_channel.groupby("date")["applications"].sum()) if not apps_by_channel.empty else [0,0,0,0,0,0,0])
 s_cti=ss.get("spark_cti",[max(cti*0.8,0), cti*0.9, cti, cti*1.05])
 s_ttr=ss.get("spark_ttr",[ttr*1.1, ttr, max(ttr*0.9,0)])

 c1,c2,c3=st.columns(3)
 _ana_kpi(c1,"Applications", f"{total_apps:,}","Últimos 30 días", s_apps)
 _ana_kpi(c2,"Contact-to-Interest Rate", f"{cti:.1f}%","Promedio", s_cti)
 _ana_kpi(c3,"Time to Reply (días)", f"{ttr:.1f}","Mediana", s_ttr)

 with st.container():
  l,r=st.columns([2,1],gap="small")
  with l:
   st.markdown('<div class="ana-card">**Applications by Hiring Channel (%)**</div>', unsafe_allow_html=True)
   if not apps_by_channel.empty:
    fig = px.bar(apps_by_channel, x="date", y="applications", color="channel", barnorm="percent")
    fig.update_layout(height=280, margin=dict(l=12,r=12,t=8,b=8), legend_title=None, plot_bgcolor="#fff", paper_bgcolor="#fff")
    st.plotly_chart(fig, use_container_width=True)
   else:
    st.info("Sin datos aún.")
  with r:
   st.markdown('<div class="ana-card">**Outreach Funnel Conversion**</div>', unsafe_allow_html=True)
   if not outreach_funnel.empty:
    ...
    st.info("Sin datos del embudo.")

 with st.container():
  a,b,c = st.columns(3, gap="small")
  with a:
   st.markdown('<div class="ana-card">**Reply Rate por canal**</div>', unsafe_allow_html=True)
   if not reply_rate.empty:
    fig = px.bar(reply_rate, x="rate", y="channel", orientation="h")
    fig.update_layout(height=260, margin=dict(l=12,r=12,t=8,b=8), plot_bgcolor="#fff", paper_bgcolor="#fff", showlegend=False)
    st.plotly_chart(fig, use_container_width=True)
   else:
    st.info("Sin datos de reply rate.")
  with b:
   st.markdown('<div class="ana-card">**Contact-to-Interest por canal**</div>', unsafe_allow_html=True)
   if not cti_rate.empty:
    fig = px.bar(cti_rate, x="rate", y="channel", orientation="h")
    fig.update_layout(height=260, margin=dict(l=12,r=12,t=8,b=8), plot_bgcolor="#fff", paper_bgcolor="#fff", showlegend=False)
    st.plotly_chart(fig, use_container_width=True)
   else:
    st.info("Sin datos de CTI.")
  with c:
   st.markdown('<div class="ana-card">**Notas**</div>', unsafe_allow_html=True)
   st.caption("Este panel se alimenta de tus otras pestañas (puestos, candidatos, outreach, etc.).")

def _section_rightstart():
 _ana_css()
 st.subheader("Analítica de Inicio")
 # Filtros de tiempo estilo chips
 rango = st.radio("Rango de tiempo", ["Hoy","Ayer","7D","30D","3M","6M","12M","Personalizado"], horizontal=True, key="rs_range")
 start, end = None, None
 today = date.today()
 if rango=="Hoy": start=end=today
 elif rango=="Ayer": start=end=today-timedelta(days=1)
 elif rango=="7D": start, end = today-timedelta(days=7), today
 elif rango=="30D": start, end = today-timedelta(days=30), today
 elif rango=="3M": start, end = today-timedelta(days=90), today
 elif rango=="6M": start, end = today-timedelta(days=180), today
 elif rango=="12M": start, end = today-timedelta(days=365), today
 else:
  col1,col2=st.columns(2)
  start = col1.date_input("Desde", today-timedelta(days=30))
  end   = col2.date_input("Hasta", today)

 engaged_pct = float(ss.get("kpi_engaged_pct", 41.7))
 tffc_days   = float(ss.get("kpi_tffc_days", 14.5))
 engaged_cnt = int(ss.get("kpi_engaged_users", 43))
 less_cnt    = int(ss.get("kpi_less_engaged_users", 60))

 c1,c2,c3,c4 = st.columns([1,1,1,1])
 _ana_kpi(c1,"Usuarios con engagement (%)", f"{engaged_pct:.1f}%")
 _ana_kpi(c2,"Tiempo hasta primer contacto (días)", f"{tffc_days:.1f}")
 _ana_kpi(c3,"Usuarios con engagement", f"{engaged_cnt}")
 _ana_kpi(c4,"Usuarios con menor engagement", f"{less_cnt}")

 # Engagement por usuario
 dfu = _df_from_state("analytics_engagement_by_user", ["user","engagement","date"])
 if start and end and "date" in dfu.columns and not dfu.empty:
  try:
   dfu["date"] = pd.to_datetime(dfu["date"]).dt.date
   dfu = dfu[(dfu["date"]>=start)&(dfu["date"]<=end)]
  except Exception:
   pass
 if not dfu.empty:
  st.markdown('<div class="ana-card">**Engagement por usuario**</div>', unsafe_allow_html=True)
  fig = px.bar(dfu.sort_values("engagement", ascending=True), x="engagement", y="user", orientation="h", color_discrete_sequence=[PRIMARY])
  fig.update_layout(height=320, margin=dict(l=12,r=12,t=8,b=8), plot_bgcolor="#fff", paper_bgcolor="#fff", showlegend=False)
  st.plotly_chart(fig, use_container_width=True)
 else:
  st.info("Aún no hay datos de engagement por usuario.")

def page_analytics():
 st.header("Analytics")
 _section_sourcing_perf()
 st.markdown("---")
 _section_rightstart()



def _render_tasks_page(title, key_filter="all"):
 st.header(title)
 tasks = ss.get("tasks", [])
 if not tasks:
  ...
  return
 if key_filter=="me":
  me = ss.get("auth",{}).get("user")
  tasks = [t for t in tasks if t.get("assignee")==me]
 elif key_filter=="team":
  my_team = ss.get("team","default")
  tasks = [t for t in tasks if t.get("team")==my_team]
 elif key_filter=="archived":
  tasks = [t for t in tasks if t.get("archived")]
 df = pd.DataFrame(tasks)
 st.dataframe(df, use_container_width=True, height=320)

def page_tasks_all(): _render_tasks_page("Todas las tareas","all")
def page_tasks_me(): _render_tasks_page("Asignadas a mí","me")
def page_tasks_team(): _render_tasks_page("Asignadas a mi equipo","team")
def page_tasks_archived(): _render_tasks_page("Archivadas","archived")
...
ROUTES = {
 ...
 "tasks_archived": page_tasks_archived,
