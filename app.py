...
def load_json(path: Path, default):
  ...
  # Si no existe, crea el archivo con los datos por defecto
...
def render_task_row(task: dict):
  ...
  def handle_action_change(task_id):
      ...
      action = ss[selectbox_key] # Obtener acción seleccionada usando la key

      # Encontrar la tarea real en la lista de sesión para actualizarla
      task_to_update = next((t for t in ss.tasks if t.get("id") == task_id), None)
      if not task_to_update: return # Si no se encuentra la tarea, no hacer nada

      if action == "Ver detalle":
          ss.expanded_task_id = task_id
          ss.show_assign_for = None
          # No se necesita rerun aquí, Streamlit lo maneja
      elif action == "Asignar tarea":
          ss.show_assign_for = task_id
          ss.expanded_task_id = None
          # No se necesita rerun aquí inicialmente
          ...
          # st.rerun() # Rerun explícito para feedback inmediato
      elif action == "Eliminar":
          ss.tasks = [t for t in ss.tasks if t.get("id") != task_id]
          save_tasks(ss.tasks)
          ss.show_assign_for = None
          ss.expanded_task_id = None
      ...
      # Experimental: Resetear el selectbox después de procesar la acción
      # Puede causar problemas si el callback se ejecuta antes de que Streamlit termine su ciclo
      # ss[selectbox_key] = "Selecciona…"

  with col5:
      selectbox_key = f"accion_{t_id}"
      # Asegurarse de que el índice sea 0 si el estado indica que no hay acción activa
      current_action_index = 0
      # Si expanded_task_id o show_assign_for corresponden a esta tarea,
      # podríamos querer mantener la selección visualmente, pero es complejo.
      # Por simplicidad, siempre volver a "Selecciona..." podría ser más estable.
      ...
      st.selectbox(
          ...
          index=current_action_index, # Intentar resetear visualmente
          on_change=handle_action_change,
          args=(t_id,)
      )


  # --- Lógica de asignación inline (sin cambios) ---
  if ss.show_assign_for == t_id:
    with st.container():
      c1,c2,c3 = st.columns([1.5,1.5,1])
      with c1:
        assign_type = st.selectbox("Tipo de Asignación", ["En Espera", "Equipo", "Usuario"], key=f"type_{t_id}", index=2)
      with c2:
        nuevo_assignee = ""
      ...
      with c3:
        if st.button("Guardar", key=f"btn_assign_{t_id}", use_container_width=True):
          # Encontrar la tarea real para actualizarla
          ...
          if task_to_update:
              ...
              if assign_type == "En Espera":
                  ...
                  # Si estaba en espera, la saca de ahí
                  if task_to_update["status"] == "En Espera":
                      task_to_update["status"] = "Pendiente"

              save_tasks(ss.tasks) # Guardar cambio
              ss.show_assign_for = None # Ocultar formulario inline

              if assign_type == "Equipo":
                  st.success("Asignado a Equipo. Redirigiendo...")
                  st.session_state.section = "agent_tasks" # Redirige
                  st.rerun() # Necesario para redirigir y refrescar
              else:
                  st.success("Cambios guardados.")
                  st.rerun() # Necesario para refrescar

def create_task_from_flow(name:str, due_date:date, desc:str, assigned:str="Coordinador RR.HH.", status:str="Pendiente"):
  t = {
    "id": str(uuid.uuid4()),
  ...
  # Asegurarse de que ss.tasks es una lista antes de insertar
  if not isinstance(ss.tasks, list):
      ss.tasks = []
  ss.tasks.insert(0, t)
  save_tasks(ss.tasks) # Guardar tarea creada


# =========================================================
# INICIALIZACIÓN DE CANDIDATOS (MODIFICADO)
# =========================================================
if "candidate_init" not in ss:
  ...
  for i, c in enumerate(initial_candidates):
    ...
    # (MODIFICADO) Usar bytes de PDF válidos
...
# =========================================================
# PÁGINAS (Sin cambios significativos en las páginas, solo ajustes menores si eran necesarios)
# =========================================================
def page_def_carga():
    ...
    if not my_tasks_filtered.empty:
        st.dataframe(
            my_tasks_filtered.rename(
                columns={"titulo":"Título", "desc":"Descripción", "due":"Vencimiento", "assigned_to": "Asignado a", "status": "Estado", "created_at": "Fecha de Creación"}
            ...
            if wf.get("status")=="Pendiente de aprobación" and puede_aprobar:
              ...
              with a1:
                if st.button("✅ Aprobar"):
                  ...
                  # Al aprobar, crear (si no existe) una tarea de ejecución
      ...
      if save_draft or send_approval or schedule:
        ...
        if not jd_final.strip(): st.error("Debes proporcionar un JD (texto o archivo).")
          ...
          if save_draft:
            ...
            # Opcional: tarea en proceso para completar diseño del flujo
...
def page_create_task():
    ...
    # --- Iterar sobre la lista de tareas (ss.tasks) ---
    ...
    # Agregar 'En Espera' si no está presente, para el filtro
    ...
    for task in tasks_to_show:
        ...
        render_task_row(task) # Renderiza la fila, incluyendo el selectbox con callback
        st.markdown('</div>', unsafe_allow_html=True)
...
if require_auth():
    ...
    # Guarda el ID de la tarea para mostrar el diálogo ANTES de renderizar la página
    task_id_for_dialog = ss.get("expanded_task_id")

    # Renderiza la página principal según la sección actual
    ROUTES.get(ss.section, page_def_carga)()

    # DESPUÉS de renderizar la página, verifica si hay que mostrar el diálogo
    if task_id_for_dialog:
        # Busca la tarea correspondiente en la lista actual de tareas
        task_data = next((t for t in ss.tasks if t.get("id") == task_id_for_dialog), None)

        if task_data:
            # Muestra el diálogo si la tarea se encontró
            with st.dialog("Detalle de Tarea", width="large"):
                ...
                st.markdown(f"""
                - Tarea creada el {task_data.get('created_at', 'N/A')}
                ...
                with st.form("comment_form"):
                    ...
                    if submitted:
                        ...
                        # Lógica para guardar comentario iría aquí

                if st.button("Cerrar", key="close_dialog"):
                    ss.expanded_task_id = None # Limpia el estado para ocultar el diálogo
                    st.rerun() # Re-ejecuta para que el diálogo desaparezca
        else:
            # Si el ID estaba en el estado pero la tarea ya no existe (ej. eliminada),
            # limpia el estado para evitar intentar abrir el diálogo.
            if ss.expanded_task_id == task_id_for_dialog: # Asegurarse de que el estado no cambió mientras tanto
               ss.expanded_task_id = None
               st.rerun() # Re-ejecuta para reflejar que el diálogo no debe mostrarse
