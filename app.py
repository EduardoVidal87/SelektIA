...
def page_agents():
  ...
  # CORRECCIÓN DE ERROR: Añadimos un chequeo de si ss.agents está inicializado y no vacío.
  if not ss.agents:
    ...
    with st.form("wf_form"):
      ...
      if save_draft or send_approval or schedule:
        ...
        if not jd_final.strip(): st.error("Debes proporcionar un JD (texto o archivo).")
          ...
          # LÓGICA DE CREACIÓN DE TAREA ASOCIADA AL FLUJO (MEJORA SOLICITADA)
          # Aseguramos que ss.agents[agent_idx] exista antes de acceder a .get()
          ...
          ss.tasks.append({
              ...
              "desc": f"El flujo '{name}' ha sido ejecutado/programado. Tarea: {desc[:50]}...",
              "due": str(run_date),
              "assigned_to": agent_role, # Asignación dinámica
              "status": "Pendiente"
          })
