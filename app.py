...
# =========================================================
# CSS (solo ajustamos lo pedido: botones del cuerpo izquierda
# y bajar/alinear los grupos del sidebar con los títulos)
# =========================================================
CSS = f"""
:root {{
  --green: {PRIMARY};
...
.sidebar-brand .brand-sub {{
  margin-top: 2px; color: {PRIMARY};  /* <-- color del logo */
  font-size: 12px; opacity: .95;
}}

/* Contenedor de navegación: lo "bajamos" para alinear con títulos */
.sidebar-nav {{
  /* Si quieres ajustar más/menos, cambia SOLO este valor */
  margin-top: 18px;      /* ↓ baja el bloque de botones */
}}
/* Reducimos separaciones verticales */
.sidebar-nav h4 {{
...
def _read_file_as_text(uploaded_file) -> str:
    """Lee PDF o TXT y devuelve texto."""
...
def _extract_meta_from_text(txt: str) -> dict:
    """Heurísticas muy simples para demo: años, universidad+anio, roles."""
    low = txt.lower()

    # Años de experiencia (búsqueda de '20+ years', '10 years', 'años')
    years = None
    m = re.search(r'(\d{1,2})\s*\+\s*(?:years|años)', low)
    if not m:
        m = re.search(r'(\d{1,2})\s*(?:years|años)\s+of\s+(?:total\s+)?experience', low)
    if not m:
        m = re.search(r'(\d{1,2})\s*(?:years|años)', low)
    if m:
        years = int(m.group(1))
    else:
        # fallback suave
        years = 5 if ("experience" in low or "experiencia" in low) else 0

    # Universidad (líneas con 'Universidad' o 'University' …) + año 19xx/20xx
    uni = None
    year_grad = None
    uni_pat = r'((?:universidad|university)[^\n,;]*)'
    m_uni = re.search(uni_pat, low, flags=re.IGNORECASE)
    if m_uni:
        uni = m_uni.group(1).strip().title()
    m_year = re.search(r'(19|20)\d{2}', low)
    if m_year:
        year_grad = m_year.group(0)

    # Roles / Cargos (líneas con 'engineer', 'developer', 'manager', etc.)
    roles = []
    for line in txt.splitlines():
        l = line.strip()
        if re.search(r'(engineer|developer|architect|manager|consultant|analista|jefe|coordinador)', l, flags=re.IGNORECASE):
            roles.append(l)
        if len(roles) >= 6:
            break
    roles = list(dict.fromkeys(roles))[:6]  # únicos, máx 6

    return {
        "anios_exp": years,
        "universidad": uni,
        "anio_grado": year_grad,
        "roles": roles
    }
...
with st.sidebar:
    ...
    # Todo el menú dentro de este contenedor para poder "bajarlo"
    ...
    if st.button("Agentes", key="sb_agents"):
        ss.section = "agents"
    if st.button("Tareas de Agente", key="sb_agent_tasks"):
    ...
    for label, sec in nav:
        if st.button(label, key=f"sb_{sec}"):
            ss.section = sec
    ...
    st.markdown('</div>', unsafe_allow_html=True)  # cierre .sidebar-nav
...
def page_def_carga():
    ...
    if files:
        ...
        for f in files:
            f_bytes = f.read()
            f.seek(0)
            raw_text = _read_file_as_text(f)
            score, reasons = simple_score(raw_text, jd_text, kw_text)
            meta = _extract_meta_from_text(raw_text)
            meta["ultima_actualizacion"] = datetime.today().date().isoformat()
...
def page_pipeline():
    ...
    # ======== Layout aprobado: lista izquierda, detalle derecha ========
    ...
    with c2:
        ...
        st.markdown(f"**Años de experiencia:** {meta.get('anios_exp','—')}")
        uni = meta.get("universidad")
        yr  = meta.get("anio_grado")
        if uni or yr:
            st.markdown(f"**Universidad:** {uni or '—'}  \n**Año:** {yr or '—'}")
        roles = meta.get("roles") or []
        if roles:
            st.markdown("**Cargos / roles (recientes)**")
            for r in roles[:6]:
                st.markdown(f"- {r}")

        st.markdown(f"**Última actualización CV:** {meta.get('ultima_actualizacion','—')}")
...
def page_agents():
    ...
    with st.form("agent_form"):
        ...
        herramientas = st.multiselect("Herramientas habilitadas", ["Parser de PDF","Recomendador de skills","Comparador JD-CV"],
